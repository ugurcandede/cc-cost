...
const en = {
    help: `cc-cost: API-equivalent cost of your Claude Code usage, across all your machines
...
Setup:
  ...
  --remove           Remove the schedule and the hook`,
    ...
    plan: {
        ...
        yours: 'Your plan ({plan}, ${price}/month): {multiple} its price in API terms over this range.',
    ...
    setup: {
        ...
        npx: 'cc-cost is running from a temporary npx folder, so a scheduler or hook would point at a path that disappears. Install it first: npm i -g cc-cost',
...
const tr: Dict = {
    help: `cc-cost: Claude Code kullanımının API karşılığı, tüm makinelerin toplamı
...
Kurulum:
  ...
  --remove           Zamanlamayı ve hook'u kaldır`,
    ...
    col: {
        ...
        item: 'Kalem', total: 'Toplam', price: 'Input / Write 5dk / Write 1sa / Read / Output ($/MTok)',
        agent: 'Agent', skill: 'Skill', mcp: 'MCP sunucusu', effort: 'Effort', context: 'Context', readCost: 'Cache read $',
        days: 'Gün', perMonth: '30 günde', limitHits: 'Limit aşımı', start: 'Başlangıç', end: 'Bitiş', tokens: 'Token',
        state: 'Durum',
    },
    item: { read: 'cache read', write: 'cache write', output: 'output', input: 'uncached input', web: 'web arama' },
    ins: {
        overview: 'Genel',
        cost: 'Maliyet',
        calls: 'API çağrısı',
        ...
        hitRatioValue: 'prompt token\'larının {pct} kadarı cache\'ten geldi',
        readShare: 'Cache read',
        writeShare: 'Cache write',
        ofCost: 'maliyetin {pct} kadarı',
        oneHour: 'maliyetin {pct} kadarı (1 saatlik katman: {tier})',
        contextSize: 'Context boyutu',
        above200k: '200K\'nın üzerindeki context\'in cache read maliyeti yaklaşık {cost} (kaba tahmin).',
        ...
        fast: 'Fast mode',
        thinking: 'Thinking',
        thinkingValue: 'output token\'larının {pct} kadarı',
        when: 'Ne zaman',
    ...
    plan: {
        ...
        yours: 'Planın ({plan}, aylık ${price}): bu aralıkta API karşılığı fiyatının {multiple} katı.',
    ...
    setup: {
        ...
        npx: 'cc-cost geçici bir npx klasöründen çalışıyor; zamanlayıcı ve hook silinecek bir yolu gösterir. Önce kur: npm i -g cc-cost',
    ...
    dash: {
        ...
        cacheHit: 'Cache isabeti', cacheHitNote: 'prompt token\'larının',
        ...
        whereCostSub: 'Token türüne göre. Cache read tipik olarak en büyük kalem; cache olmasa aynı input 10 katına faturalanırdı.',
        itemsLabel: 'Token türüne göre maliyet',
        cacheRead: 'Cache read', cacheWrite: 'Cache write', output: 'Output', input: 'Uncached input', web: 'Web arama',
